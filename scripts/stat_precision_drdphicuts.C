...
void stat_precision_drdphicuts(){


///READ TEPX number of Clusters per event for pu=200
  float TEPX_C=0;
  float TEPXDR_C[2][4][5];
  TFile FTEPX_C("/home/ashish/Desktop/26thOct2020_28thOct2020presentation_drdphicuts_2sigma/TEPX/samples_17Feb2020/2023D42PU200.root","r");
  std::vector<string> disk_C={"1","2","3","4"};
  std::vector<string> side_C={"1","2"};

  for(long d=0;d<4;d++){
for(long s=0;s<2;s++){
TString histoname = "BRIL_IT_Analysis/TEPX/Clusters/Numberofclusters_side";
   TH2F* H1 = (TH2F*)FTEPX_C.Get(histoname + side_C[s].c_str() + "_Disk" + disk_C[d].c_str());
...
///READ TEPX number of real 2x coincidences  per event for pu=200
  ...
  TFile FTEPX_2x("/home/ashish/Desktop/26thOct2020_28thOct2020presentation_drdphicuts_2sigma/TEPX/samples_17Feb2020/2023D42PU200.root","r");
  std::vector<string> disk_2x={"1","2","3","4"};
std::vector<string> side_2x={"1","2"};

  for(long d=0;d<4;d++){
for(long s=0;s<2;s++){
TString histoname = "BRIL_IT_Analysis/TEPX/2xCoincidences/Numberof2xCoincidences_side";
   TH2F* H2 = (TH2F*)FTEPX_2x.Get(histoname + side_2x[s].c_str() + "_Disk" + disk_2x[d].c_str());
  ...
  cout<<"\\hline"<<endl;
  print_precision2("TEPXD4R1 Clusters",TEPXDR_C[1][0][0]+TEPXDR_C[1][7][0],800e3);
  print_precision2("TEPXD4R1 2x Coincidences ",TEPXDR_2x[1][0][0]+TEPXDR_2x[1][7][0],800e3);
  ...
  cout<<"\\hline"<<endl;  
  print_precisionvdm("TEPXD4R1 Clusters",TEPXDR_C[1][0][0]+TEPXDR_C[1][7][0],800e3);  
  print_precisionvdm("TEPXD4R1 2x Coincidences ",TEPXDR_2x[1][0][0]+TEPXDR_2x[1][7][0],800e3);
