void plotting_code_2xdrdphicuts(){
  ...
  TFile *f = new TFile("/home/ashish/BRIL-upgrade/tdrplots/TDRplots2xinphi.root","RECREATE");

  f->cd();
  gDirectory->pwd();
  f->ls();
    
  TString outputpath1 = "./2xCoincidences/Extrapolation/";
  TString outputpath2 = "./2xCoincidences/2xCoincidences_Fit/";  
  TString outputpath3 = "./2xCoincidences/AllPU_residuals/";
  ...
  NonLinearity_TEPXClustersPerEvent[0][3][0]->SetMarkerColor(2);
 ...
 C1.Print(outputpath3 + "2xinphiD4R1(-Z).gif");
 ...
 C4.Print(outputpath3 + "2xinphiD4R1(+Z).gif");
 ...
 for (int s = 0; s < sidelist.size(); s++) {
   for (int d = 0; d < disklist.size(); d++) {
     ...
     C2.Print(outputpath3 + TString("2xCoincidencesdisk_2xinphi") + d + s + ".gif");
